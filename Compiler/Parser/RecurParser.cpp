...
namespace PARSER {
    ...
    void Reverse_Polish(std::vector<std::pair<int, std::string>>token_buffer) {
        ...
        6. &&
        7. ||
        which is the same as the rule of C++
        */
        std::string ans = "";
        std::vector<std::string>temp_stack;//stack for op
        for (int i = 0; i < token_buffer.size(); i++) {
            if(token_buffer[i].second==";"){
                while (!temp_stack.empty()) {
                    ans += temp_stack.back();
                    temp_stack.erase(temp_stack.end() - 1);
                }
                std::cout << ans << '\n';
                ans = "";
            }
            else {
                if (token_buffer[i].first == 1 || token_buffer[i].first == 7 || token_buffer[i].first == 3) {
                    ans += token_buffer[i].first;
                }
                else if(token_buffer[i].second == "(") { 
                    temp_stack.push_back(token_buffer[i].second); 
                }               
                else if(token_buffer[i].second == ")"){
                    ...
                    if (!temp_stack.empty() && temp_stack.back() != "(" &&
                        temp_stack.back() != ")" && Prior(temp_stack.back(), token_buffer[i].second)) {
                        ans += temp_stack.back();
                        temp_stack.erase(temp_stack.end()-1);
                    }
                    else {
